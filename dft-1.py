...
def idft2(rex, imx):
    ...
    for i in range(N):
        XX[i] = fsum([REX[k]*cos(2*pi*k*i/N) +
...
# - possibly more efficient as fsum() takes any iterable as its
...
def dft2(XX):
    ...
    for k in range(len(REX)):
        REX[k] =  fsum([XX[i]*cos(2*pi*k*i/N) for i in range(N)])
        IMX[k] = -fsum([XX[i]*sin(2*pi*k*i/N) for i in range(N)])
...
def dft3(XX):
    ...
    N = len(XX) # N is the number of points in XX
    K = fft_size
    REX = [fsum([+XX[i]*cos(2*pi*k*i/N) for i in range(N)])
           for k in range(K)]
    IMX = [fsum([-XX[i]*sin(2*pi*k*i/N) for i in range(N)])
           for k in range(K)]
