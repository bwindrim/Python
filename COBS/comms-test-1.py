...
def test_variance(src, channel_mode=1, limit=1):
    try:
        query_channel_mode(src)
        set_channel_mode(src, channel_mode)
        variance = get_variance(src)
...
def run_test(dev1, test, *args):
    ...
    with serial.Serial(dev1, BaudRate, timeout=0.5) as ser1:  # open serial port
        ...
        with ReaderThread(ser1, Porp) as src:  # reader thread to handle incoming packets from src
            ...
            enable_coding_mode(src, 0x1ACFFC1D)
            set_rx_scaling(src) # switch to binary quantisation for testing (direct wired)
            good, bad =  test(src, *args) # pass the trailing arguments to the test function
...
for test in test_list:
    ...
    for mode in mode_list:
        usb0[mode] = run_test(sys.argv[1], test, mode, repeats)
